...
describe("Transaction", () => {
  ...
  describe("updateTransaction()", () => {
    ...
    describe("`amount` is valid", () => {
      ...
      it("deducts `nextAmount` from `latestSenderBalance` of `senderWallet`", () => {
        expect(transaction.outputMap[senderWallet.publicKey]).toEqual(
          latestSenderBalance - nextAmount
        );
      });
      ...
      describe("update to the same `recipient`", () => {
        ...
        it("deducts the `amount` from `senderWallet` address for same-recipient", () => {
          expect(transaction.outputMap[senderWallet.publicKey]).toEqual(
            latestSenderBalance - nextAmount - newAmount
          );
        });
      });
    });
  ...
  describe("rewardTransactions()", () => {
    ...
    it("reward `transaction` is created and the input is `REWARD_INPUT`", () => {
      expect(rewardTransaction.input).toEqual(REWARD_INPUT);
    });

    it("reawrdTransactin amount is `MINING_REWARD`", () => {
