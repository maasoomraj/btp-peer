...
class BlockChain {
  ...
  isValidChain(chain) {
    ...
    for (var i = 1; i < chain.length; i++) {
      ...
      if (
        presentBlock.lastHash !== lastBlock.hash ||
        presentBlock.hash !== Block.hashBlock(presentBlock)
      )
        return false;
    }
      ...
      for (let transaction of block.data) {
        if (transaction.input.address === SENDER_INPUT.sender_address) {
          if (transaction.input.from === receiver_transaction.input.from) {
            if (
              transaction.input.product === receiver_transaction.input.product
            ) {
              if (
                transaction.input.amount === receiver_transaction.input.amount
              ) {
